        ...
        if (solverChoice.use_real_bcs && solverChoice.init_type == InitType::WRFInput) {
            ...
            if (n_time_old > 0) {
                ...
                read_and_convert_from_wrfbdy(itime,nc_bdy_file,
                                             ...
                                             r_hse, area_vec, geom[0], use_moist, solverChoice.rebalance_wrf_input,
                                             solverChoice.maintain_Th, domain_bcs_type,
...
ERF::ReadParameters ()
{
    ...
    ParmParse pp_amr("amr");
    {
        ...
        if (num_ground_stations > 0) {
            const bool have_station0 =
                pp.countval("doppler_lidar.station0.i_loc") > 0 ||
                pp.countval("doppler_lidar.station0.j_loc") > 0 ||
                pp.countval("doppler_lidar.station0.interval") > 0 ||
                pp.countval("doppler_lidar.station0.height") > 0 ||
                pp.countval("doppler_lidar.station0.file_name") > 0;
            const bool have_stationN =
                pp.countval("doppler_lidar.station" + std::to_string(num_ground_stations) + ".i_loc") > 0 ||
                pp.countval("doppler_lidar.station" + std::to_string(num_ground_stations) + ".j_loc") > 0 ||
                pp.countval("doppler_lidar.station" + std::to_string(num_ground_stations) + ".interval") > 0 ||
                pp.countval("doppler_lidar.station" + std::to_string(num_ground_stations) + ".height") > 0 ||
                pp.countval("doppler_lidar.station" + std::to_string(num_ground_stations) + ".file_name") > 0;

            if (have_station0 && have_stationN) {
                Abort("Do not mix zero-based and one-based erf.doppler_lidar station numbering");
            }

            const int station_offset = have_station0 ? 0 : 1;
            ground_stations.resize(num_ground_stations);
            for (int n = 0; n < num_ground_stations; ++n) {
                const std::string prefix = "doppler_lidar.station" + std::to_string(n + station_offset);
