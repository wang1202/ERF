...
namespace
{
    ...
    {
        if (k <= 0) {
            return amrex::max(zt(ic,0,0) - zi(ic,0,0), shoc_eps());
        }
        if (k >= layout.nlev) {
            return amrex::max(zi(ic,layout.nlev,0) - zt(ic,layout.nlev-1,0), shoc_eps());
        }
        return amrex::max(zt(ic,k,0) - zt(ic,k-1,0), shoc_eps());
    }
                                     ...
                                     const ShocRuntimeOptions& opts)
{
    ...
    ParallelFor(col_box, [=] AMREX_GPU_DEVICE (int ic, int, int) noexcept
    {
        ...
        for (int k = 1; k < layout.nlev; ++k) {
            ...
            const Real dz_zt_k = amrex::max(dz(ic,k,0), shoc_eps());
            const Real dz_zt_km1 = amrex::max(dz(ic,k-1,0), shoc_eps());
            const Real dz_zi = cell_spacing_at_iface(zt, zi, layout, ic, k);
            ...
            const Real a0 = (0.52_rt / (c * c)) / amrex::max(c - 2.0_rt, shoc_eps());
            const Real a1 = 0.87_rt / (c * c);
            const Real a2 = 0.5_rt / c;
            const Real a3 = 0.6_rt / (c * amrex::max(c - 2.0_rt, shoc_eps()));
            ...
            const Real y1 = (2.0_rt * a2 * (buoy_sgs2 * x1 + (a0 / amrex::max(a1, shoc_eps())) * f0 + f1)) /
